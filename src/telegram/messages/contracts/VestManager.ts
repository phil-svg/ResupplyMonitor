...
function prisma(address: string) {
  if (address.toLowerCase() === address_PRISMA.toLowerCase()) return hyperlink_Prisma();
  if (address.toLowerCase() === address_yPrisma.toLowerCase()) return hyperlink_yPrisma();
  if (address.toLowerCase() === address_cvxPrisma.toLowerCase()) return hyperlink_cvxPrisma();
}
...
export async function getMessage_VestManager_Claimed(event: any): Promise<string | null> {
  ...
  return `
🎉${await getUser(event.returnValues.account)} claimed ${formatForPrint(
    event.returnValues.amount / 1e18
  )}${hyperlink_RSUP()} from Vesting
...
export async function getMessage_VestManager_TokenRedeemed(event: any): Promise<string | null> {
  ...
  return `
🔁${await getUser(event.returnValues.redeemer)} redeemed ${formatForPrint(event.returnValues.amount / 1e18)}${prisma(
...
export async function getMessage_VestManager_VestCreated(event: any): Promise<string | null> {
  console.log(event);
  return null;
}
export async function getMessage_VestManager_AirdropClaimed(event: any): Promise<string | null> {
  ...
  const allocationTypeName =
    allocationTypeMap[event.returnValues.allocationType] ?? `Unknown Type (${event.returnValues.allocationType})`;

  return `
🎁${await getUser(event.returnValues.account)} claimed ${formatForPrint(
    event.returnValues.amount / 1e18
  )}${hyperlink_RSUP()} (vested) from ${allocationTypeName}
Circulating Supply: ${formatForPrint(circulatingSupplyRSUP)}${hyperlink_RSUP()}  
${lastLine}  
  `;
}

export async function getMessage_VestManager_ClaimSettingsSet(event: any): Promise<string | null> {
