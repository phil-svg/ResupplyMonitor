...
async function getApr_sreUSD(blockNumber: number): Promise<string> {
...
async function getTotalAssets_sreUSD(event: any) {
...
export async function getMessage_sreUSD_Deposit(event: any): Promise<string | null> {
  ...
  const callerNote =
    event.returnValues.caller?.toLowerCase() !== event.returnValues.owner?.toLowerCase() ? ` (by ${caller})` : '';
...
export async function getMessage_sreUSD_Withdraw(event: any): Promise<string | null> {
  ...
  const receiverNote =
    event.returnValues.receiver?.toLowerCase() !== event.returnValues.owner?.toLowerCase() ? ` to ${receiver}` : '';

  const lastLine = await getLastLine(event.transactionHash, event.blockNumber);

  return `
User${owner}${callerNote} withdrew ${formatForPrint(
    assets
  )}${hyperlink_reUSD()} from${hyperlink_sreUSD()}${receiverNote}
sreUSD Total Assets: ${formatForPrint(Number(totalAssets) / 1e18)}${hyperlink_reUSD()} | APR: ${apr}
${lastLine}  
  `;
}
